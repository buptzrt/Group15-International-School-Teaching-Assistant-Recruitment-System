...
//如果大模型还没反应过来，它就会一直在那里安静地等待，绝不会卡死整个页面。
// ==========================================--%>
...
<td>
    <div style="display: flex; gap: 8px;">
        ...
        <% } else { %>
        <a class="apply-btn" href="javascript:void(0);" onclick="event.stopPropagation(); confirmApply('<%= job.getJobId() %>', '<%= job.getJobTitle().replace("'", "\\'") %>', event, '<%= totalAccepted %>', '<%= jobHrs %>')">Apply</a>
        <% } %>
    </div>
</td>
</tr>
...
alert("⏳ AI 正在深度思考中，请等待表格中的 Loading 结束！");
return;
}

// 🌟 自动排版优化：把换行变成 <br>，把分类标题加粗标蓝
...
<%-- // ==========================================
// 🌟 终极 JSP 兼容版加载器：彻底消灭 EL 表达式冲突

//在现代前端开发中，我们习惯用反引号配合“美元符号+左右中括号”来拼接字符串，比如。

//使用的是 JSP (JavaServer Pages)！Tomcat 在把页面发送给浏览器之前，会先像扫雷一样把整个 .jsp 文件扫一遍。它看到时“美元符号+左右中括号”，会误以为这是 Java 里的 EL 表达式，想要去找后端的 Java 变量。

//因为 Java 后端根本没有叫 data.score 的属性，Tomcat 就会直接把它们替换成空字符串！等你打开浏览器时，原本的 JS 代码已经被 Tomcat 阉割成了 `%`。

//在 JSP 文件里写 JS 时，千万不要用反引号和“美元符号+左右中括号”去拼变量！ 必须老老实实地用加号（+）和双引号来拼接，才能防止 Tomcat “多管闲事”。
// ==========================================--%>
async function loadAiScores() {
const tbody = document.querySelector("#jobTable tbody");
if (!tbody) return;
const rows = Array.from(tbody.querySelectorAll("tr.job-row")); // 只抓取主行
...
mainRows.sort((a, b) => {
const scoreA = parseInt(a.getAttribute('data-aiscore')) || -1;
const scoreB = parseInt(b.getAttribute('data-aiscore')) || -1;
