...
public class StudentResumeUploadServlet extends HttpServlet {
    ...
    // ✅ 移除原本写死的静态 Path，改为在 doPost 内部动态解析
    private static final String SRC_RESOURCES_PATH = "E:\\Group15_TA_SYSTEM-Cqt-pdfchange\\TA_System\\src\\main\\resources\\";
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            // ✅ 核心修改点：动态确定上传路径列表（双写：SRC 和 TARGET）
            List<Path> uploadPaths = new ArrayList<>();
            uploadPaths.add(Paths.get(SRC_RESOURCES_PATH).resolve(RESUME_RELATIVE_DIR).normalize());
