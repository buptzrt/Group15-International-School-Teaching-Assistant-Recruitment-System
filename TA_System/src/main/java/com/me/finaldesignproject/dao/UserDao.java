...
public class UserDao {
    ...
    private static String getUserJsonPath() {
        // 使用resources目录作为存储位置
        return "E:/Group15_TA_SYSTEM/TA_System/src/main/resources/" + USER_JSON_FILE;
    }
    ...
    /**
     * ✅ 新增：根据学号/ID 从 users.json 中获取完整的 User 对象
     */
    public User getUserByEnrollment(String enrollmentNo) {
        if (enrollmentNo == null) return null;

        // 获取所有用户（复用你现有的 getAllUsers 方法）
        List<User> allUsers = getAllUsers();

        if (allUsers != null) {
            for (User user : allUsers) {
                // 匹配学号（忽略大小写和空格）
                if (enrollmentNo.trim().equalsIgnoreCase(user.getEnrollmentNo())) {
                    return user;
                }
            }
        }
